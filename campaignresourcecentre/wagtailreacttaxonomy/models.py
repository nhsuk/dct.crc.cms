...
def get_terms_from_children(children, vocab_code, index_path, level):
    ...
    for obj in children:
        if obj.get("type") == "term":
            index_path_for_level = (
                obj.get("code")
                if index_path == ""
                else "%s|%s" % (index_path, obj.get("code"))
            )
            terms[obj.get("code")] = {
                "label": obj.get("label"),
                "vocabCode": vocab_code,
                "indexPath": index_path_for_level,
                "level": level,
            }
            children = obj.get("children", None)
            if children:
                next_level = level + 1
                child_terms = get_terms_from_children(
                    children, vocab_code, index_path_for_level, next_level
                )
                terms.update(child_terms)
    return terms
    ...
    def clean(self):
        ...
        if not topic_vocab:
            raise ValidationError(
                {"taxonomy_json": "No topic vocabulary found in taxonomy"}
            )
        ...
        if not any(item["code"] in topics for item in tags):
            raise ValidationError(
                {"taxonomy_json": "Please select at least one topic tag"}
            )
...
class PageTaxonomyPermissionsMixin(models.Model):
    global_permission = models.CharField(
        max_length=100, null=True, blank=True, default="public"
    )
...
def format_permissions_json(sender, instance, **kwargs):
    ...
    for group_key, groups in (json.loads(instance.permissions_json)).items():
        ...
        for action_key, vocs in groups.items():
            permissions_json_formatted[group_key].extend(
                ["{0}.{1}".format(action_key, voc) for voc in vocs]
            )

...
class PermissionsPanel(FieldPanel):
    ...
    class BoundPanel(FieldPanel.BoundPanel):
        ...
        def load_permission_terms(self):
            ...
            try:
                data = TaxonomyTerms.objects.get(
                    taxonomy_id=self.panel.permission_terms_id
                )
                try:
                    json.loads(data.terms_json)
                except json.decoder.JSONDecodeError:
                    self.permission_terms_error_message = (
                        '"Permission Terms" json wrong format'
                    )
                permission_terms_json = data.terms_json
            except TaxonomyTerms.DoesNotExist:
                self.permission_terms_error_message = (
                    'No "Permission Terms" for this id: "{}"'.format(

...
class ParagonUsersTestCase(WagtailPageTests):
    ...
    def test_user_profile_postcode_is_backwords_compatible(self, mock_client_class):
        ...
        test_cases = [
            {"postcode_data": "SW1A 1AA|London", "expected": "SW1A 1AA"},
            {"postcode_data": "M1 1AA", "expected": "M1 1AA"},
        ]
    ...
    @patch("campaignresourcecentre.paragon_users.views.Client")
    def test_signup_create_account_with_new_fields(
        self,
        mock_client_class,
        mock_get_region,
        mock_send_report,
        mock_get_postcode_data,
        mock_send_verification,
    ):
        self.client.logout()
        mock_get_region.return_value = "Test Region"
        mock_get_postcode_data.return_value = {
            "longitude": 0,
            "latitude": 0,
            "region": "Test Region",
        }
        ...
        with patch(
            "campaignresourcecentre.paragon_users.views.RegisterForm"
        ) as mock_form_class:
            mock_form = mock_form_class.return_value
            mock_form.is_valid.return_value = True

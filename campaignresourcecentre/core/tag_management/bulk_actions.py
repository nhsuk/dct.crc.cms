...
class ManageTagsBulkAction(PageBulkAction):
    ...
    def get_execution_context(self):
        return {
            **super().get_execution_context(),
            ...
            'source_page_id': self.cleaned_form.cleaned_data.get('source_page').id if self.cleaned_form.cleaned_data.get('source_page') else None,
    ...
    def form_valid(self, form):
        ...
        template = 'tag_management/results_remove.html' if active_tab == 'remove' else 'tag_management/results_copy.html'
    ...
    @staticmethod
    def track_changes(instance, page, original, final, removed, added, mode, had_changes=True):
        if instance:
            instance.change_details.append({
                'page_id': page.id,
                'page_title': page.title,
                'page_url': page.specific.url if hasattr(page, 'specific') else '',
                'edit_url': getattr(page, 'edit_url', f'/crc-admin/pages/{page.id}/edit/'),
    ...
    def copy_tags_to_page(cls, page, source_tags, mode, user, instance=None):
        ...
        cls.track_changes(instance, page, current, final, removed, added, mode, had_changes)
        return had_changes

    @classmethod
    def execute_action(cls, objects, active_tab='remove', tags_to_remove='{}', source_page_id=None, tag_operation_mode='merge', user=None, **kwargs):
        ...
        if active_tab == 'copy':
            ...
            for page in objects:
                try:
                    if cls.copy_tags_to_page(page, source_tags, tag_operation_mode, user, instance):
                        num_modified += 1
    ...
    def get_success_message(self, num_modified, num_failed):
        ...
        return ngettext(f"{num_modified} page updated", f"{num_modified} pages updated", num_modified)

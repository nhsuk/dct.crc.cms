...
from django.utils.cache import patch_vary_headers, get_cache_key, add_never_cache_headers
...
def remove_vary_headers (response, headersToRemove):
    ...
    if existingHeaders:
        vary_headers = [
            headerName for headerName in cc_delim_re.split(existingHeaders)
...
def is_cacheable (request, response, path):
    ...
    if cc and 'private' in cc: return False
    # Don't cache any page rendered for a logged-in Django/Wagtail user
    if hasattr (request, "user") and request.user.is_authenticated: return False
    ...
    return path in (
        "/",
        "/campaigns/"
    ) or (campaign_page_re.match (path) is not None)
    ...
    def process_response(self, request, response):
        ...
        if canCache:
            if len (response.cookies):
                logger.info ("Potentially cachable page %s set cookies %s", path, sorted (response.cookies.items ()))
...
class CRCFetchFromCacheMiddleware (FetchFromCacheMiddleware):
    ...
    def process_request(self, request):
        ...
        if response is not None:
            ...
            if len (response.cookies):
                logger.error ("Cached page %s set cookies %s", path, sorted (response.cookies.items ()))

...
from campaignresourcecentre.standardpages.views import\
    contact_us, thank_you, clear_cache, session_summary,\
    cookie_confirmation, cookie_settings, update_index
...
private_urlpatterns = [
    ...
    path(
        "account/newsletters/", newsletter_preferences, name="newsletter_preferences"
    ),
    ...
    path(
        "orders/place/", orders_views.place_order, name="place_order"
    ),
    ...
    path("cookie_settings/", standardpages_views.cookie_settings, name="cookie_settings"),
    path("cookie_confirmation/", standardpages_views.cookie_confirmation, name="cookie_confirmation")
]
...
urlpatterns = decorate_urlpatterns(
    urlpatterns,
    vary_on_headers(
        "X-Requested-With", "X-Forwarded-Proto", "Accept-Encoding"
    ),
)

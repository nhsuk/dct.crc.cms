...
def problem_error_list_page(context):
    ...
    for error in expected_error_list:
        assert_that(any(error in s for s in actual_error_list), equal_to(True),
...
def forgot_password_validation(context):
    expected_forgot_pwd_error_list = create_list_from_feature_table_column(context, 'error_list')
    ...
    for error in expected_forgot_pwd_error_list:
        assert_that(any(error in s for s in actual_forgot_pwd_error_list), equal_to(True),
                    f"error link as not as expected: {error}")
...
@Step('Research behind this campaign and how to use this campaign expand and collapse for "{source}"')
...
@Step('I enter Register details of "{FirstName}" "{LastName}" "{Org_Name}" "{Postcode}" "{Email}" "{Password}"')
def CRCV3_Register(context, FirstName, LastName, Org_Name, Postcode, Email, Password):
    context.support_page.Register_form(FirstName, LastName, Org_Name, Postcode, Email, Password)
...
@Step("Verify all register empty fields validation errors displayed in the problem error list")
def Register_problem_list_page(context):
    expected_Register_problem_error_url_list = create_list_from_feature_table_column(context, 'problem_error_list')
    actual_Register_error_list = context.support_page.return_empty_register_errors_link_url()
...
@Step("Verify all register invalid fields validation errors displayed in the problem error list")
def Register_problem_list_page(context):
    expected_Register_problem_error_url_list = create_list_from_feature_table_column(context, 'problem_error_list')
    actual_Register_error_list = context.support_page.return_invalid_register_errors_link_url()
    for error in expected_Register_problem_error_url_list:
        assert_that(any(error in s for s in actual_Register_error_list), equal_to(True),
                    f"error link as not as expected: {error}")
...
@Step("I browsed to Better Health Start for Life Introducing Solid Foods resource campaign")
...
def address_validation(context):
    ...
    for error in expected_error_lists:
        assert_that(any(error in s for s in actual_error_lists), equal_to(True),
                    f"error link as not as expected: {error}")
...
def Invalid_Email(context):
    expected_error_lists = create_list_from_feature_table_column(context, 'Invalid_email')
    actual_error_lists = context.support_page.Empty_email_error()
    for error in expected_error_lists:
        assert_that(any(error in s for s in actual_error_lists), equal_to(True),
                    f"error link as not as expected: {error}")
...
@Step("I enter Email address field with invalid_email and click submit button then verify invalid_email_error")
def Empty_email(context):
    invalid_email_lists = create_list_from_feature_table_column(context, 'invalid_email')
    for email in invalid_email_lists:
        context.support_page.forgot_password_email(email)
        context.support_page.submit_button()
        expected_error_lists = create_list_from_feature_table_column(context, 'invalid_email_error')
        actual_error_lists = context.support_page.Empty_email_error()
        for error in expected_error_lists:
            assert_that(any(error in s for s in actual_error_lists), equal_to(True),
                    f"error link as not as expected: {error}")
